...
const execute = async function ({destDir = process.cwd(), answers} = {}) {
  ...
  if (resulted_answers['dnaCellDirPath'] && resulted_answers['dnaCellDirPath'] !== '.') {
    resulted_answers['cell-branch'] = `cells.${resulted_answers['dnaCellDirPath'].split('/').join('.')}.${resulted_answers['cell-name']}`
  } else {
    resulted_answers['cell-branch'] = `cells.${resulted_answers['cell-name']}`
  }
